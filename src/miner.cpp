...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("UnnMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("UnnMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("UnnMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("unn-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running UnnMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
