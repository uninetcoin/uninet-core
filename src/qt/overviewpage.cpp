...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::UNN)
    {
    }
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("UNN")
    updateDisplayUnit();
}
...
void OverviewPage::updatePrivatesendProgress()
{
    ...
    QString strAnonymizeUnnAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeUnnAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeUnnAmount = strAnonymizeUnnAmount.remove(strAnonymizeUnnAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeUnnAmount + " / " + tr("%n Rounds", "", nPrivatesendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeUnnAmount * COIN) nMaxToAnonymize = nAnonymizeUnnAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeUnnAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeUnnAmount));
        strAnonymizeUnnAmount = strAnonymizeUnnAmount.remove(strAnonymizeUnnAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeUnnAmount + " / " + tr("%n Rounds", "", nPrivatesendRounds);
    } else {
